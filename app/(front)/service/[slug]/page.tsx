...
import { DoctorDetail, serviceResponse } from '@/types/types';
...
export default async function page({params:{slug},searchParams}:{
  ...
  const data=await(await getData(`services/${slug}`)).data
...
<div className="py-3 flex flex-col text-sm space-y-2">
  {services.map((item,i)=>{
    return (
<Link key={i} href={`/service/${item.slug}`} className='hover:to-blue-600'>{item.title}</Link>
    )
  })}
