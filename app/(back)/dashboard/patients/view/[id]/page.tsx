...
export default async function page({params:{id}}:{params:{id:string}}) {
    ...
    if(user){
      appointments=  await getData(`appointments/patients/detail/${id}`)
    }
  ...
  return (
    <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mt-5 px-3'>
    {
        appointments.map((item,i)=>{
            ...
            return(
                <Link key={i} href={ `/dashboard/patients/detail/${item.id}`} className={cn('border mb-2 border-gray-300 shadow-sm text-xs  py-4 px-3 inline-block w-full rounded-md dark:text-slate-100 gap-4',)}>
